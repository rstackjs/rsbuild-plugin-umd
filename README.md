...
By default, the UMD plugin will output a `dist/index.js` file. You can modify the name of the output file through Rsbuild's [output.filename](https://rsbuild.rs/config/output/filename) option.
...
If you need filename hash, enable the [output.filenameHash](https://rsbuild.rs/config/output/filename-hash) option:
...
If you need to generate HTML files, you can set [tools.htmlPlugin](https://rsbuild.rs/config/tools/html-plugin) to `true`:
...
If you need to split the UMD outputs, you can actively configure [performance.chunkSplit](https://rsbuild.rs/config/performance/chunk-split), for example:
